...
vec4 effect(vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords) 
{
    ...
    float is_vertical_line = step(mod(screen_coords.x + cameraCoords.x + offset.x, gridSize), 1.0);
    float is_horizontal_line = step(mod(screen_coords.y + cameraCoords.y + offset.y, gridSize), 1.0);
